...
const root = await fs.mkdtemp(path.join(os.tmpdir(), 'localbridge-mcp-p3-'));
...
const client = new Client({ name: 'localbridge-mcp-p3-test', version: '0.0.1' });
...
try {
  ...
  const writeTool = listed.tools.find(tool => tool.name === 'lb_write_text');
  ...
  const multi = jsonOf(await call(client, 'lb_read_many_texts', {
    file_paths: [path.join(workspace, 'alpha.txt'), path.join(workspace, 'second.txt')]
  }));
  assert.equal(multi.length, 2);
  assert.match(multi[1].content, /second file/);

  const listing = jsonOf(await call(client, 'lb_list_entries', { directory_path: workspace }));
  assert(listing.some((x: any) => x.name === 'alpha.txt'));
  const info = jsonOf(await call(client, 'lb_stat_path', { target_path: path.join(workspace, 'alpha.txt') }));
  assert.equal(info.isFile, true);

  const nested = path.join(workspace, 'created', 'nested');
  assert.match(textOf(await call(client, 'lb_make_directory', { directory_path: nested })), /CREATED/);
  const writePath = path.join(nested, 'write.txt');
  assert.match(textOf(await call(client, 'lb_write_text', {
    file_path: writePath,
    text: 'ONE\n',
    write_mode: 'rewrite'
  })), /WROTE/);
  await call(client, 'lb_write_text', { file_path: writePath, text: 'TWO\n', write_mode: 'append' });
  assert.equal(await fs.readFile(writePath, 'utf8'), 'ONE\nTWO\n');

  const advisoryPath = path.join(nested, 'advisory.txt');
  const advisory = textOf(await call(client, 'lb_write_text', {
  ...
  await call(client, 'lb_move_path', { from_path: writePath, to_path: movedPath });
  ...
  const editExact = jsonOf(await call(client, 'lb_patch_text_block', {
    file_path: editPath,
    find_text: 'brave old',
  ...
  const editFuzzy = jsonOf(await call(client, 'lb_patch_text_block', {
    file_path: editPath,
  ...
  const searchStart = jsonOf(await call(client, 'lb_search_start', {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    searchPage = jsonOf(await call(client, 'lb_search_read', {
      search_id: searchId,
      result_offset: 0,
      result_count: 100
    }));
  ...
  const searches = jsonOf(await call(client, 'lb_search_sessions'));
  assert(searches.some((s: any) => s.sessionId === searchId));
  const stop = jsonOf(await call(client, 'lb_search_cancel', { search_id: searchId }));
  assert.equal(stop.stopped, true);
  console.log('MCP_SEARCH_TOOLS_PASS');

  const blocked = await call(client, 'lb_run_shell', { command_line: 'sudo -n true', wait_ms: 250 });
  assert.equal(blocked.isError, true);
  assert.match(textOf(blocked), /blocked/i);

  const persistent = jsonOf(await call(client, 'lb_run_shell', {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    initialOutput = jsonOf(await call(client, 'lb_shell_output', {
  ...
  const interaction = await call(client, 'lb_shell_input', {
    process_id: persistent.pid,
    stdin_text: 'hello'
  });
  ...
  const output = jsonOf(await call(client, 'lb_shell_output', {
    process_id: persistent.pid,
    line_offset: -20,
    line_count: 20
  }));
  ...
  const sessions = jsonOf(await call(client, 'lb_shell_sessions'));
  assert(sessions.active.some((s: any) => s.pid === persistent.pid));

  const terminated = await call(client, 'lb_shell_kill', { process_id: persistent.pid });
  assert.match(textOf(terminated), /TERMINATION_REQUESTED/);
  console.log('MCP_TERMINAL_TOOLS_PASS');

  const escape = await call(client, 'lb_read_text', { file_path: '/etc/hosts' });
  assert.equal(escape.isError, true);
  assert.match(textOf(escape), /outside allowed directories/i);
  console.log('MCP_POLICY_BOUNDARY_PASS');

  assert.match(stderr, /LocalBridge MCP ready on stdio/);
